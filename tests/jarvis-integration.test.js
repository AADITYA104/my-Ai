...
(async () => {
  ...
  const result = await runJarvisAgent("complete deterministic smoke task", {
    planner: async () => ({ goal: "complete deterministic smoke task", assumptions: [], steps: [{ id: 1, description: "Produce the expected result", doneWhen: "result is OK", tool: null, input: {}, risk: "low" }] }),
  ...
  const listedRead = registry.list().find(tool => tool.name === "read_file");
  assert.strictEqual(listedRead.inputSchema.file_path.required, true);
  assert.strictEqual(listedRead.inputSchema.file_path.type, "string");
  ...
  await assert.rejects(
    executeWithPolicy({ tool: "read_file", input: {}, risk: "low" }, { autoApprove: false }),
    err => err.code === "INVALID_TOOL_INPUT" && /file_path/.test(err.message)
  );
  await assert.rejects(
    executeWithPolicy({ tool: "run_code", input: { language: "ruby", code: "1 + 1" }, risk: "low" }, { autoApprove: false }),
    err => err.code === "INVALID_TOOL_INPUT" && /language/.test(err.message)
  );
  ...
  const directory = await executeWithPolicy({ tool: "list_directory", input: { dir_path: "." }, risk: "low" }, { autoApprove: false });
  assert.ok(Array.isArray(directory));
  assert.ok(directory.some(entry => entry.name === "package.json"));

  await assert.rejects(executeWithPolicy({ tool: "list_directory", input: { dir_path: "../" }, risk: "low" }, { autoApprove: false }), err => err.code === "WORKSPACE_ESCAPE");
  ...
  await assert.rejects(executeWithPolicy({ tool: "run_command", input: { command: "echo safe" }, risk: "high" }, { autoApprove: true }), err => err.code === "APPROVAL_REQUIRED");

  process.env.JARVIS_ALLOW_AUTO_APPROVE = "true";
  assert.strictEqual(isServerAutoApprovalEnabled(), true);
  const approvedOutput = await executeWithPolicy({ tool: "run_command", input: { command: "echo safe" }, risk: "high" }, { autoApprove: true });
  ...
  const safeOutput = await executeWithPolicy({ tool: "read_file", input: { file_path: "package.json" }, risk: "low" }, { autoApprove: false });
  ...
  await executeWithPolicy(readStep, { autoApprove: false, loopGuard });
  await assert.rejects(executeWithPolicy(readStep, { autoApprove: false, loopGuard }), err => err.code === "LOOP_GUARD_BLOCKED");
