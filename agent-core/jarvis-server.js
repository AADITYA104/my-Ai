...
const TOOL_SCHEMAS = {
  read_file: { file_path: { type: "string", required: true } },
  write_file: { file_path: { type: "string", required: true }, content: { type: "string", required: true } },
  run_command: { command: { type: "string", required: true } },
  run_code: { language: { type: "string", required: false, enum: ["javascript", "python"] }, code: { type: "string", required: true } },
  list_directory: { dir_path: { type: "string", required: false } },
  design_audit: { target: { type: "string", required: true } },
  hierarchical_crew: { mission: { type: "string", required: true }, maxSteps: { type: "number", required: false } },
  debate_group_chat: { topic: { type: "string", required: true }, maxTurns: { type: "number", required: false } },
  generate_3d_model: {
    text: { type: "string", required: false }, imagePath: { type: "string", required: false },
    texture: { type: "boolean", required: false }, outputPath: { type: "string", required: false }
  }
};
  ...
  registry.register({
    ...
    execute: (input) => {
      if (name === "run_command") {
        ...
        return execSync(command, { cwd: process.cwd(), encoding: "utf-8", timeout: 15000, maxBuffer: 1024 * 1024, windowsHide: true, stdio: ["ignore", "pipe", "pipe"] });
      }
      if (name === "list_directory") {
        const dirPath = resolveWorkspacePath(input.dir_path || ".");
        return fs.readdirSync(dirPath, { withFileTypes: true }).map((entry) => ({ name: entry.name, type: entry.isDirectory() ? "directory" : entry.isFile() ? "file" : "other" }));
      }
      if (name === "run_code") return executeTool("code_exec", adaptLegacyToolInput(name, input));
      return executeTool(name, adaptLegacyToolInput(name, input));
    }
  });
}

function isServerAutoApprovalEnabled() { return process.env.JARVIS_ALLOW_AUTO_APPROVE === "true"; }
function normalizeSessionId(value) { return typeof value === "string" && value.trim() ? value.trim() : "default_session"; }
...
async function executeWithPolicy(step, executionContext = {}) {
  if (!step || typeof step.tool !== "string" || !step.tool.trim()) throw new Error("A valid tool name is required.");
  const input = step.input && typeof step.input === "object" ? step.input : {};
  const policyContext = { root: process.cwd(), sideEffectClass: step.risk === "high" ? "external_side_effect" : undefined, autoApprove: executionContext.autoApprove === true && isServerAutoApprovalEnabled() };
  const loopGuard = executionContext.loopGuard instanceof AgentLoopGuard ? executionContext.loopGuard : new AgentLoopGuard();
  ...
  if (anomaly.isLoop) {
    ...
    error.code = anomaly.type === "approval_required" ? "APPROVAL_REQUIRED" : anomaly.policy?.reason === "Path is outside the configured workspace." ? "WORKSPACE_ESCAPE" : "LOOP_GUARD_BLOCKED";
...
app.get("/health", (_req, res) => res.json({ service: "JARVIS", status: "ONLINE", timestamp: new Date().toISOString(), tools: registry.list() }));
...
app.post("/api/jarvis/run", async (req, res) => {
  ...
  try {
    ...
    const result = await runJarvisAgent(goal, { ...createExecutionContext(autoApprove, loopGuard), sessionId, taskId: normalizeTaskId(req.body.taskId) || undefined, limits: req.body.limits || {}, autoApprove, availableTools: registry.list() });
