...
# Bug fix: worker.generate() runs in a background thread (see /send below).
# If it raises -- e.g. texture requested but the server wasn't started with
# --enable_tex, so self.pipeline_tex doesn't exist -- the exception was
# previously swallowed by the thread with no trace, and /status kept
# returning {"status": "processing"} forever since it only checks whether
# the output file exists. Callers had no way to tell "still working" apart
# from "crashed silently" except waiting out their own client-side timeout.
job_errors = {}
...
async def generate(request: Request):
    ...
    def _run_and_capture_errors(uid, params):
        try:
            worker.generate(uid, params)
        except Exception as e:
            logger.error(f"Background generation failed for {uid}: {e}")
            traceback.print_exc()
            job_errors[str(uid)] = str(e)

    threading.Thread(target=_run_and_capture_errors, args=(uid, params,)).start()
    ret = {"uid": str(uid)}
...
async def status(uid: str):
    if uid in job_errors:
        return JSONResponse({'status': 'error', 'message': job_errors[uid]}, status_code=200)
